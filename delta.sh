...
case "$1" in
    help|-h|--help)
        ...
        ;;
    list)
        echo -e "${yellow}Available systems:${reset}"
        echo "adelie.sh alpine.sh archlinux.sh artix.sh chimera.sh debian.sh deepin.sh fedora.sh manjaro.sh opensuse.sh pardus.sh rockylinux.sh ubuntu.sh void.sh kali/kali.sh windows11.sh"
        exit 0
        ;;
    update)
    ...
    "")
        ...
        case "$choice" in
            ...
            13) FILE="ubuntu.sh" ;;
            14) FILE="void.sh" ;;
            15) FILE="kali/kali.sh" ;;   # Kali special path
